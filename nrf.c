...
static ssize_t nrf_read(struct file *fp, char *buf, size_t len, loff_t *offset){
	...
	*offset += strlen(s);	
	...
	print_registers(buf, &count, offset);

	// ------------------------- set mode RX ------------------------------------------------------------
	// spiWriteRegister(RH_NRF24_REG_00_CONFIG, _configuration | RH_NRF24_PWR_UP | RH_NRF24_PRIM_RX);
	// digitalWrite(_chipEnablePin, HIGH);
	// arg_val[0] = PWR_UP | PRIM_RX;
			//nrf_xfer(CONFIG, 1, arg_val, W);
		//os registos ja estao como recv //// so falta o CE
